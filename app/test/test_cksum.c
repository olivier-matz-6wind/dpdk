 ...
 * ", ".join("0x{:02x}".format(ord(c)) for c in str(Ether()/IP()/TCP()))
 */
...
/* generated in scapy with
 * ", ".join("0x{:02x}".format(ord(c)) for c in str(Ether()/IPv6()/TCP()))
 */
...
/* generated in scapy with
 * ", ".join("0x{:02x}".format(ord(c)) for c in str(Ether()/IP()/UDP()/Raw('x')))
 */
...
/* generated in scapy with
 * ", ".join("0x{:02x}".format(ord(c)) for c in str(Ether()/IPv6()/UDP()/Raw('x')))
 */
...
/* generated in scapy with
 * ", ".join("0x{:02x}".format(ord(c))
 *   for c in str(Ether()/IP(options='\x00')/UDP()/Raw('x')))
 */
...
test_l4_cksum(struct rte_mempool *pktmbuf_pool, const char *pktdata, size_t len)
{
	...
	if (l3 == RTE_PTYPE_L3_IPV4 || l3 == RTE_PTYPE_L3_IPV4_EXT) {
		...
		if (l4 == RTE_PTYPE_L4_TCP) {
			...
			prev_cksum = udp->dgram_cksum;
			udp->dgram_cksum = 0;
			udp->dgram_cksum = rte_ipv6_udptcp_cksum(l3_hdr, l4_hdr);
			if (udp->dgram_cksum != prev_cksum)
				GOTO_FAIL("invalid TCP checksum calculation");
			udp->dgram_cksum = prev_cksum;
		}
